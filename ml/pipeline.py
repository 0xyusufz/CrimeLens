...
def process_document(
    ...
    **kwargs: Any,
    ...
    if len(args) == 3:
        ...
        if isinstance(doc_content, bytes):
            if not is_scanned_file(str(filename)):
                # Not an image extension — decode as text
                try:
                    text = doc_content.decode("utf-8")
                except UnicodeDecodeError:
                    text = doc_content.decode("latin-1")
            else:
                # Extension looks like image/PDF — check actual magic bytes
                # Real images start with known magic bytes; plain text PDFs do not
                IMAGE_MAGIC = (
                    b"\xff\xd8\xff",   # JPEG
                    b"\x89PNG",         # PNG
                    b"GIF8",             # GIF
                ...
                is_real_image = any(doc_content[:8].startswith(m) for m in IMAGE_MAGIC)
                if is_real_image:
                    text = doc_content  # True binary image → OCR path
                else:
                    # PDF or other text-like bytes → decode as text
                    try:
                        text = doc_content.decode("utf-8")
                    except UnicodeDecodeError:
                        try:
    ...
    if isinstance(text, bytes):
        # Only run OCR on real image magic bytes; otherwise treat as text
        IMAGE_MAGIC_BYTES = (
            b"\xff\xd8\xff",   # JPEG
            b"\x89PNG",         # PNG
            b"GIF8",             # GIF
        ...
        is_real_image_bytes = any(text[:8].startswith(m) for m in IMAGE_MAGIC_BYTES)
        if is_real_image_bytes:
            raw_text = extract_text_from_image(text, lang=cfg.default_ocr_language, engine_runner=ocr_runner)
        else:
            try:
                raw_text = text.decode("utf-8")
            except UnicodeDecodeError:
                raw_text = text.decode("latin-1", errors="replace")
    elif isinstance(text, str):
        import os
        # Only send to OCR if it's an actual file path that exists on disk
        if is_scanned_file(text) and os.path.isfile(text):
            raw_text = extract_text_from_image(text, lang=cfg.default_ocr_language, engine_runner=ocr_runner)
        else:
