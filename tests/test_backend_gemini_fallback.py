"""Backend E2E Gemini fallback test: API → DB → Neo4j.

Verifies that when the REAL PersonBDocumentProcessor is used with Gemini enabled,
but Gemini fails (503/timeout), the FastAPI endpoint still returns 200 OK,
successfully persists deterministic entities to the database, and does not leave
...
class BackendGeminiFallbackTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def setUp(self):
        ...
        # Enable Gemini at the environment level so the real ML pipeline picks it up
        ...
        created = client.post(
            ...
            json={"title": f"Fallback API case {uuid.uuid4().hex[:8]}"},
        )
    ...
    @patch("ml.ai.providers.gemini.subprocess.run")
    def test_gemini_503_fallback_persists_successfully(self, mock_run):
